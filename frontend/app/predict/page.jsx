...
export default function PredictPage() {
  ...
  const [methods, setMethods] = useState(['bert', 'cosine'])
  ...
  useEffect(() => {
    ...
    }, Math.max(800, 4000 / stages.length))

    return () => clearInterval(stageInterval)
  }, [loading, methods])

  // Helper function to remove nested arrays from objects
  const sanitizeForFirestore = (obj) => {
    if (obj === null || obj === undefined) return null
    if (typeof obj !== 'object') return obj
    
    if (Array.isArray(obj)) {
      // Convert array to string if it contains objects or arrays
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (value === null || value === undefined) {
        ...
        // Check if array contains objects or nested arrays
        if (value.some(item => typeof item === 'object' && item !== null)) {
          // Convert to JSON string to avoid nested array issues
  ...
  const handleSubmit = async e => {
    ...
    if (!authChecked || !user) {
      setSaveError('Please log in to save your analysis history.')
    } else if (!user.emailVerified) {
      setSaveError('Your email is not verified. Please verify your email to save results.')
    } else {
      setSaveError('')
    }
    ...
    try {
      ...
      if (user && user.emailVerified) {
        // STEP 1: Save ranking summary
        const rankingsCollection = collection(db, 'users', user.uid, 'rankings')
        
        const topCandidate = data?.results && Array.isArray(data.results) 
          ? data.results.find(r => r.rank === 1)?.filename || null
          : null

        const rankingDocRef = await addDoc(rankingsCollection, {
        ...
        console.log('Ranking doc created:', rankingDocRef.id)

        // STEP 2: Save ALL resume details in subcollection (with sanitization)
        if (data?.results && Array.isArray(data.results) && data.results.length > 0) {
          const detailsCollection = collection(db, 'users', user.uid, 'rankings', rankingDocRef.id, 'details')
          
          console.log('Saving details for', data.results.length, 'resumes')
          
          const savePromises = data.results.map(async resumeResult => {
            // Prepare safe data for Firestore (no nested arrays)
            const safeData = {
              filename: resumeResult.filename || null,
              rank: resumeResult.rank || null,
              finalScore: resumeResult.final_score || null,
              weightedScore: resumeResult.weighted_score || null,
              
              // Simple scores object
              scores: resumeResult.scores || {},
              
              // Text fields
              explanation: resumeResult.explanation || null,
              
              // Simple array of strings
              extractedSkills: Array.isArray(resumeResult.extracted_skills) 
                ? resumeResult.extracted_skills.slice(0, 50) // Limit to 50 skills
                : [],
              
              // File info object
              fileInfo: resumeResult.file_info || {},
              
              // Convert complex nested data to JSON strings
              algorithmDetailsJson: resumeResult.algorithm_details 
                ? JSON.stringify(sanitizeForFirestore(resumeResult.algorithm_details))
                : null,
              
              // Store full result as JSON string (safest approach)
              fullResultJson: JSON.stringify(sanitizeForFirestore(resumeResult)),
              
              createdAt: serverTimestamp(),
            }

            return await addDoc(detailsCollection, safeData)
          })

          await Promise.all(savePromises)
          console.log('All details saved successfully!')

          setSaveSuccess(`✅ Analysis complete! Saved ${data.results.length} resume results to your account.`)
        } else {
          console.warn('No results to save')
          setSaveSuccess('✅ Analysis complete! (No results to save)')
        }
      }
    } catch (e) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-indigo-100/50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
      ...
      <div className="container py-12 relative z-10">
        <div className="text-center mb-12 animate-fade-in">
          ...
          {authChecked && user && !user.emailVerified && (
            <div className="mt-4 text-yellow-600 font-semibold">
              Your email is not verified. Verify your email to save results and access history.
            </div>
          )}
          {authChecked && !user && (
            <div className="mt-4 text-red-600 font-semibold">
              You are not logged in. Log in to save your analysis results.
            </div>
          )}
          {!authChecked && (
          ...
          {saveSuccess && (
            <div className="mt-4 rounded border border-green-500 bg-green-100 px-4 py-3 text-green-700">
              {saveSuccess}
            </div>
          )}
          {saveError && (
            <div className="mt-4 rounded border border-red-500 bg-red-100 px-4 py-3 text-red-700">
              {saveError}
            </div>
          )}
        </div>
              ...
              return (
                <div key={index} className="flex items-center">
                    ...
                    <div className="text-center mt-2 hidden md:block">
                      <div
                        className={`text-sm font-medium ${
                          step.active ? 'text-primary-600 dark:text-primary-400' : 'text-gray-500'
                        }`}
                      >
                        {step.title}
                      </div>
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`hidden md:block w-16 h-0.5 transition-all duration-500 ${
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="card animate-slide-up" style={{ animationDelay: '0.1s' }}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Job Description *</label>
                  <textarea
...
We are looking for a Senior Software Engineer with 3+ years of experience in React, Node.js, and cloud technologies. The ideal candidate should have strong problem-solving skills and experience with agile development...`}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Position Type *</label>
                      ...
                      <label key={pos.value} className="cursor-pointer">
                        ...
                        >
                          ...
                          <span className={`font-medium ${
                            position === pos.value ? 'text-primary-700 dark:text-primary-300' : 'text-gray-700 dark:text-gray-300'
                          }`}>{pos.label}</span>
                        </div>
                      </label>
                    ))}
              ...
              <div className="flex items-center mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    Choose which algorithms to use for comprehensive analysis
                  </p>
                </div>
                {methods.length > 0 && (
                  <div className="ml-auto flex items-center text-sm text-primary-600 dark:text-primary-400">
                ...
                {loading ? (
                  <>
                    ...
                    <span className="relative z-10">
                      Processing with {methods.length} Algorithm{methods.length !== 1 ? 's' : ''}...
                    </span>
                  </>
                ) : (
                  <>
              ...
              {loading && (
                <div className="mt-4 p-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl border border-primary-200 dark:border-primary-800">
                  <div className="flex items-center justify-center text-primary-700 dark:text-primary-300 font-medium">
                    ...
                    <div
                      className="w-2 h-2 bg-primary-500 rounded-full animate-bounce mr-2"
                      style={{ animationDelay: '0.1s' }}
                    ></div>
                    <div
                      className="w-2 h-2 bg-primary-500 rounded-full animate-bounce mr-4"
          ...
          {results && !loading && (
            <div className="mt-12 animate-fade-in">
              <div className="text-center mb-8">
                ...
                <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Resume Rankings</h2>
                <p className="text-gray-600 dark:text-gray-400">
                  Results from {methods.length} AI algorithm{methods.length !== 1 ? 's' : ''}, sorted by combined
                  confidence score
                </p>
              </div>
